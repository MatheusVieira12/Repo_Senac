...
SELECT produto, valor_unitario FROM venda;

SELECT produto, categoria, estado FROM venda
WHERE estado = 'SP';

SELECT SUM(valor_unitario * quantidade) AS total_arrecado
FROM venda;

SELECT categoria, SUM(valor_unitario * quantidade) AS valor_categoria
FROM venda
GROUP BY categoria;

SELECT cliente, valor_unitario FROM venda
WHERE valor_unitario > 3000;


SELECT produto, valor_unitario FROM venda
WHERE cidade = 'Curitiba';

SELECT cliente, SUM(valor_unitario * quantidade) AS total_gasto
FROM venda
GROUP BY cliente
HAVING total_gasto > 4000;

SELECT cliente, valor_unitario, produto, quantidade
FROM venda
WHERE valor_unitario * quantidade > 5000;

SELECT cliente, produto, SUM(valor_unitario * quantidade) AS total_gasto
FROM venda
GROUP BY cliente
HAVING total_gasto > 5000;

SELECT cliente, SUM(quantidade) FROM venda
WHERE valor_unitario > 200
GROUP BY cliente;

SELECT cidade, SUM(valor_unitario * quantidade) AS Total_vendidoEstado
FROM venda
WHERE cidade = 'Curitiba' OR cidade = 'Porto Alegre' OR cidade = 'Belo Horizonte'
